...
void Hora::operator+=(int segons) {
   incrementa(segons);
}

Hora Hora::operator+(int segons) const {
   Hora resultat(_hor, _min, _seg);
   resultat.incrementa(segons);
   return resultat;
}

bool Hora::operator<(const Hora& altra) const {
...
Hora Hora::operator-(const Hora& altra) const {
    Hora resultat;
    if (operator>(altra)) {
